...
import { BackgroundCircleBubblesImage, BackgroundCircleBubblesInverted, BackgroundCircleBubblesLightImage, BackgroundCircleBubblesLightInverted } from './../../components';
import { } from './../../actions';
...
class BackgroundImageComponent extends React.Component<Props> {
  ...
  render(): React.ReactNode {
    ...
    const bottomClass = 'background-bottom' + (this.props.svgBottom || this.props.bottomRotate ? ' rotate' : '');
    ...
    const topCircleStyle = this.props.svgTop ? {} : {
      backgroundImage: `url(${this.props.backgroundTopDark
    ...
    const bottomCircleStyle = this.props.svgBottom ? {} : {
      backgroundImage: `url(${this.props.backgroundBottomDark
        ? BackgroundCircleBubblesImage
        : BackgroundCircleBubblesLightImage})`,
      opacity: this.props.backgroundBottomOpacity,
    }

    const hasGradient = this.props.gradient && `linear-gradient(${this.props.gradient}) `;
    const hasImage = this.props.backgroundImage && `url(${this.props.backgroundImage}) `;
    const backgroundImage =
      hasGradient && hasImage
        ? `${hasGradient},${hasImage}`
        : hasGradient ? hasGradient : hasImage ? hasImage : null;

    return (
      <Fragment>
        {backgroundImage &&
          <div className={`background-image ${this.props.shadow ? 'shadow' : ""}`} style={{ backgroundImage: backgroundImage }} />
        }
